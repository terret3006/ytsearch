...
def submit_text():
    ...
    print(f"Selected option from user: {selected_option}")  # Print the selected option
    ...
    if selected_option == 'one':
        ...
        return jsonify({"message": "Invalid option selected", "table": ""})
    ...
    if all_data:
        ...
        # Create the new column for video URL with timestamp
        df['video_url_with_timestamp'] = df.apply(
            lambda row: f"{row['video_url']}&t={row['start_time_seconds']}", axis=1
        )

        # Drop the video_url column
        df.drop(columns=['video_url'], inplace=True)

        # Process URLs to plain text for Excel
        df['video_url_with_timestamp'] = df['video_url_with_timestamp'].apply(process_urls_to_text)

        # Save the DataFrame to a file for pagination use
        ...
        return jsonify({"message": f"No lines containing the keyword '{keyword}' found in any text file.", "table": ""})
...
def get_data():
    ...
    page_data['video_url_with_timestamp'] = page_data['video_url_with_timestamp'].apply(make_clickable)
    table_html = page_data.to_html(classes='table table-striped table-bordered', index=False, escape=False)
